...
<body class="bg-black text-slate-300 antialiased min-h-screen flex flex-col selection:bg-slate-700 selection:text-white relative" style="font-family: 'Inter', sans-serif;">
    ...
    <div class="fixed inset-0 z-0 overflow-hidden bg-black">
        <div class="absolute inset-0 z-0 opacity-20" style="background: radial-gradient(circle at 50% 50%, rgba(16, 185, 129, 0.15) 0%, rgba(5, 150, 105, 0.04) 45%, rgba(0, 0, 0, 0) 80%);"></div>
    ...
    <main class="flex-grow flex flex-col items-center justify-center relative z-30 px-6 py-12 min-h-screen w-full">
        ...
        <div class="mt-8 flex flex-col items-center gap-4 relative z-20">
            <div class="flex -space-x-2">
                <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&h=100&fit=crop&crop=face" alt="Active Node" class="w-10 h-10 rounded-full border border-slate-800 bg-black object-cover shadow-lg">
                <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face" alt="Active Node" class="w-10 h-10 rounded-full border border-slate-800 bg-black object-cover shadow-lg">
                <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop&crop=face" alt="Active Node" class="w-10 h-10 rounded-full border border-slate-800 bg-black object-cover shadow-lg">
                <div class="w-10 h-10 rounded-full border border-slate-700 bg-slate-900 flex items-center justify-center text-xs text-emerald-400 font-extralight shadow-lg">+</div>
            </div>
            <p class="text-xs text-slate-500 font-extralight uppercase tracking-widest">Validated by distributed consensus nodes</p>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.addEventListener('message', (e) => {
                if (e.data && e.data.type === 'trigger-explosion') {
                    explosions.push({ x: e.data.x, y: e.data.y, radius: 0, life: 1 });
                }
            });
            ...
            function render() {
                ...
                for (let i = 0; i < explosions.length; i++) {
                    ...
                    ctx.strokeStyle = 'rgba(52, 211, 153, ' + (exp.life * 0.35) + ')';
                    ctx.lineWidth = 1.2;
                    ctx.stroke();
                    ctx.restore();
...
const GATEWAY_FLOW_DEFINITION: EffectDefinition = {
  ...
  patch(source, { size, density, mode, strokeColor, particleColor }) {
    let next = source
      .replace(
        "const numPaths = 80;",
    ...
    if (mode === "light") {
      next = next
        .replace(
          "ctx.strokeStyle = 'rgba(16, 185, 129, 0.32)';",
          "ctx.strokeStyle = 'rgba(16, 101, 60, 0.4)';",
        )
        .replace(
          "ctx.fillStyle = 'rgba(52, 211, 153, 0.85)';",
...
function buildFocusedDocument(
  ...
  const patchedSource = definition.patch
    ? definition.patch(definition.source, {
        ...
        strokeWidth: knobs.strokeWidth,
        mode,
        strokeColor: knobs.strokeColor,
        particleColor: knobs.particleColor,
      })
    : definition.source;
...
function GatewayFlowFrame({
  ...
  // Forward parent window clicks to trigger canvas particle deflection/explosions
  useEffect(() => {
